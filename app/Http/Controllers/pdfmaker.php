...
class pdfmaker extends Controller
{
    ...
    public function bankCetakKredit(Request $request)
    {

        $pdf = \App::make('dompdf.wrapper');
        $pdf->loadHTML($this->dataBankKredit($request));
        return $pdf->stream();
    }

    public function dataBankKredit(Request $request)
    {
    ...
    public function bankCetakAngsuran(Request $request)
    {

        $pdf = \App::make('dompdf.wrapper');
        $pdf->loadHTML($this->dataBankAngsuran($request));
        return $pdf->stream();
    }

    public function dataBankAngsuran(Request $request)
    {
        $daritanggal = $request->daritanggal;
        $ketanggal = $request->ketanggal;
        $bank = bankModel::where('email', auth()->user()->email)->first();
